...
import { Users, UserPlus, Shield, CheckCircle, XCircle, Key, RefreshCw, Building2, Lock, Copy, X, AlertCircle } from 'lucide-react';
...
export default function UserManagementPage() {
  ...
  const handleResetPassword = async (e) => {
    ...
    if (resetModal.newPassword !== resetModal.confirmPassword) {
      setResetModal((prev) => ({ ...prev, error: 'New password and confirmation do not match' }));
      return;
    }
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/75 backdrop-blur-xs p-4 animate-in fade-in duration-150">
          <div className="w-full max-w-md bg-[#1e293b] border border-[#334155] rounded-2xl p-6 shadow-2xl space-y-4 animate-in zoom-in-95 duration-150 relative">
            <div className="flex items-start justify-between border-b border-[#334155] pb-3.5">
                ...
                <div>
                  ...
                  <div className="flex items-center gap-2 mt-1">
                    <span className="text-xs text-slate-300 font-semibold">
                      {resetModal.user?.full_name}
                    </span>
                    <span className="text-[10px] font-mono text-slate-400">
                      (@{resetModal.user?.username})
                    </span>
                    {resetModal.user?.role && (
                      <span className="text-[9px] uppercase tracking-wider px-1.5 py-0.5 rounded font-mono font-bold bg-purple-950/80 text-purple-300 border border-purple-800">
                        {resetModal.user.role}
                      </span>
                    )}
                  </div>
              ...
              <button
                type="button"
                data-testid="reset-modal-close-btn"
                onClick={() => setResetModal({ open: false, user: null, newPassword: '', confirmPassword: '', submitting: false, error: '', successData: null })}
            ...
            {resetModal.error && (
              <div className="p-3 bg-rose-950/80 border border-rose-800 text-rose-300 rounded-xl text-xs flex items-center gap-2.5 shadow-xs animate-in fade-in duration-150">
                <AlertCircle className="w-4 h-4 text-rose-400 shrink-0" />
                <span className="font-medium">{resetModal.error}</span>
              </div>
            )}

            {resetModal.successData ? (
              <div className="space-y-4 text-xs">
                <div className="p-4 bg-emerald-950/60 border border-emerald-800 text-emerald-300 rounded-xl space-y-3">
                  <div className="flex items-center gap-2 font-bold text-sm text-emerald-200">
                    <CheckCircle className="w-4 h-4 text-emerald-400 shrink-0" /> Password Reset Successfully
                  </div>
                  ...
                  <div className="flex items-center justify-between bg-[#0f172a] border border-emerald-700/80 rounded-xl p-3 mt-2 shadow-inner">
                    <span data-testid="temp-password-display" className="font-mono text-emerald-300 text-base font-bold tracking-wider select-all">
                    ...
                    <button
                      ...
                      className="px-3 py-1.5 bg-emerald-700 hover:bg-emerald-600 text-white rounded-lg font-semibold flex items-center gap-1.5 transition text-xs cursor-pointer shadow-xs"
                    >
                ...
                <div className="flex justify-end pt-2">
                  <button
                    ...
                    className="px-4 py-2 bg-[#0f172a] hover:bg-[#334155] text-white rounded-lg font-bold shadow cursor-pointer transition border border-[#334155]"
                  >
                    Done
                  </button>
                </div>
              ...
              <form onSubmit={handleResetPassword} className="space-y-3.5 text-xs">
                ...
                <div>
                  ...
                  {resetModal.confirmPassword && resetModal.newPassword && (
                    <div className="mt-1 text-[11px] font-medium flex items-center gap-1">
                      {resetModal.newPassword === resetModal.confirmPassword ? (
                ...
                <div className="p-3 bg-[#0f172a] border border-[#334155] rounded-xl text-[11px] text-slate-400 space-y-1">
                  <div className="text-slate-300 font-medium flex items-center gap-1.5">
                    <Lock className="w-3 h-3 text-amber-400" /> Administrative Security Notice
                  </div>
                  <div>• Password will be securely salted and hashed with bcrypt.</div>
